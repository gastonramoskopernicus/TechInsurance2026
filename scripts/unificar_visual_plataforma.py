...
export default function PlataformaPage() {
  return (
    <div className="flex flex-col w-full relative bg-[#020104]">
      ...
      <section id="modulos" className="py-24 px-4 bg-[#020104] border-b border-white/5 scroll-mt-32">
        <div className="container mx-auto max-w-7xl relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[
              { title: 'Product Studio', desc: 'Diseño y modelado no-code', icon: '<path d="m18 16 4-4-4-4"/><path d="m6 8-4 4 4 4"/><path d="m14.5 4-5 16"/>' },
              { title: 'Commercial Engine', desc: 'Lógica comercial core', icon: '<path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>' },
              { title: 'Data & Rules', desc: 'Decisiones y normativas', icon: '<rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/>' },
              { title: 'Partner Link', desc: 'Integración con terceros', icon: '<path d="M16 21v-2a4 4 0 0 0-4-4H5c-1.1 0-2 .9-2 2v2"/><circle cx="8.5" cy="7" r="4"/><polyline points="17 11 19 13 23 9"/>' },
              { title: 'Campaign Manager', desc: 'Gestión comercial ágil', icon: '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>' },
              { title: 'API Gateway', desc: 'Exposición y control', icon: '<rect x="4" y="4" width="16" height="16" rx="2" ry="2"/><rect x="9" y="9" width="6" height="6"/>' },
              { title: 'Embedded Layer', desc: 'Canales de distribución', icon: '<path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/>' },
            ].map((m) => (
              <div key={m.title} className="group p-8 border border-white/5 bg-[#0f0e11] rounded-2xl hover:bg-[#16141a] hover:border-fuchsia-900/50 transition-all duration-300 flex flex-col">
                 <div className="mb-4 text-zinc-600 group-hover:text-fuchsia-500 transition-colors duration-300">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-10 h-10" dangerouslySetInnerHTML={{ __html: m.icon }}></svg>
                 </div>
                 <h3 className="text-xl font-bold text-white mb-2">{m.title}</h3>
                 <p className="text-zinc-500 font-light group-hover:text-zinc-400 transition-colors">{m.desc}</p>
              </div>
            ))}
          </div>
        </div>
