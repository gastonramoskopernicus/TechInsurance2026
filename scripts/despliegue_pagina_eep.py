...
def main():
    print("Iniciando despliegue de EEP (Engagement Ecosystem Platform)...")

    # 1. Rutas
    base_dir = "/Users/gastonramos/Documents/Antigravity/tech-insurance-2026"
    eep_dir = os.path.join(base_dir, "app", "eep")
    header_path = os.path.join(base_dir, "app", "components", "Header.tsx")
    page_path = os.path.join(eep_dir, "page.tsx")
    client_path = os.path.join(eep_dir, "EepClient.tsx")

    # Crear directorio app/eep si no existe
    os.makedirs(eep_dir, exist_ok=True)
    print(f"Directorio creado o verificado: {eep_dir}")

    # 2. Contenido de page.tsx
    page_content = """import EepClient from "./EepClient";
...
"""
    ...
    print(f"Archivo de página creado: {page_path}")

    # 3. Contenido de EepClient.tsx
...
"""
    ...
    print(f"Archivo cliente de React creado: {client_path}")

    # 4. Modificación de Header.tsx
    # Debemos agregar la opción EEP inmediatamente después de InspeXiona.
    # InspeXiona está declarado como: { label: "InspeXiona", href: "/inspexiona" },
    print(f"Modificando Header.tsx en: {header_path}")
    with open(header_path, "r", encoding="utf-8") as f:
        header_text = f.read()

    # Buscamos la línea de InspeXiona
    inspexiona_pattern = r'\{\s*label:\s*"InspeXiona",\s*href:\s*"/inspexiona"\s*\},'
    eep_line = '\n    { label: "EEP", href: "/eep" },'

    # Verificamos si ya está agregado (por idempotencia)
    if 'href: "/eep"' in header_text:
        print("El menú EEP ya existe en Header.tsx. No se modifica.")
    else:
        # Reemplazamos insertando EEP después de InspeXiona
        modified_header_text = re.sub(
            inspexiona_pattern,
            lambda m: m.group(0) + eep_line,
            header_text
        )
        with open(header_path, "w", encoding="utf-8") as f:
            f.write(modified_header_text)
        print("Header.tsx modificado con éxito.")

    print("Despliegue de EEP completado exitosamente.")
