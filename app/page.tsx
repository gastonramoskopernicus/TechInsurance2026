          ...
          <div className="space-y-6 max-w-3xl">
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-extrabold tracking-tight text-white leading-[1.1]">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-500 to-pink-400">
                de su core de seguros para innovar.
              </span>
            </h1>
              ...
              <div key={arrayIndex} className="flex items-center gap-16 md:gap-32 px-8 md:px-16">
                 {[
                   ...
                   <div key={i} className={`flex-shrink-0 flex items-center justify-center p-2 h-16 w-32 md:h-20 md:w-40 opacity-70 hover:opacity-100 transition-all duration-500 hover:scale-105 hover:drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] ${logo.filterClass}`}>
                      <img src={`/empresas/${logo.file}`} alt={logo.file.split('.')[0]} className={`w-full h-full object-contain transition-transform duration-300 ${(logo as any).imageClass || ""}`} />
                   </div>
                 ))}
              </div>
            ))}
